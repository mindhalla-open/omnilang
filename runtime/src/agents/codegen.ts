...
export class CodeGenAgent {
  ...
  public async generateService(
    ...
    if (target === "rust") {
      CodeGenAgent.updateRustModFile(outputDir, serviceName);
    }
  ...
  public static updateRustModFile(outputDir: string, serviceName: string): void {
    const modRsPath = path.join(outputDir, "src", "services", "mod.rs");
    const modName = CodeGenAgent.toSnakeCase(serviceName);
